...
## 설명
- DataFrame에서 특정 행(Row)을 조건으로 필터링한다.
- df['모델'].isin([...])을 사용하여 원하는 iPhone 모델 목록을 지정한다.
- 해당 목록에 포함된 행만 True로 반환되어 필터링된다.
- iPhone 8부터 iPhone XS까지 연속된 모델 데이터만 추출된다.
- print()로 결과를 출력한다.
...
# 특정 범위 Row 선택 (iPhone 8 ~ iPhone XS)
result = df.loc[df['모델'].isin(['iPhone 8', 'iPhone X', 'iPhone XR', 'iPhone XS'])]
...
```
    ...
    Cell In[13], line 7
          4 df = pd.read_csv('iphone.csv')
          6 # 특정 범위 Row 선택 (iPhone 8 ~ iPhone XS)
    ----> 7 result = df.loc[df['모델'].isin(['iPhone 8', 'iPhone X', 'iPhone XR', 'iPhone XS'])]
          9 # 출력
         10 print(result)
...
# iPhone 8 ~ iPhone XS 범위 Row 선택
result = df.loc[df['Unnamed: 0'].isin([
    'iPhone 8',
    'iPhone 8 Plus',
    'iPhone X',
    'iPhone XR',
...
---
### ***Face ID, 출시일, 메모리의 Column(컬럼) 값 받아오기(Retry).***


```python
iphone_df[['Face ID','출시일','메모리']]      # 이 2가지 코드 중에서 제일 짧은 코드
```
...
```python
iphone_df.loc[:,['Face ID','출시일','메모리']]      #df.loc['인덱스','컬럼'] 방식
```
...
---
### ***iPhone 8부터 iPhone XS 까지의 연속적인 Row(열)(로우) 값 받아오기(Retry).***


```python
iphone_df.loc['iPhone 8':'iPhone XS']      # 짧은. 정석. 좋은. 코드.
```
...
---
### ***맨 처음부터 iPhone XS 까지의 연속적인 Row(열)(로우) 값 받아오기.***


```python
iphone_df.loc[:'iPhone XS']      # 가장 짧은 코드(없어도 되는 코드 일부 삭제)
```
...
---
### ***메모리부터 Face ID 까지의 연속적인 Cloumn (컬럼) 값 받아오기.***


```python
iphone_df.loc[:, '메모리':]      # 가장 짧은 코드
```
...
---
### ***iPhone 7 부터 iPhone X 까지의 메모리부터 Face ID 까지의 값 받아오기.***


```python
iphone_df.loc[:'iPhone X', '메모리':]      # 가장 짧은 코드(최소 코드)
```
...
# **[20260414 Tue] - 오늘 수업 마무리(2교시).**

# **[20260421 Tue] - 오늘 수업 시작(1교시).**
...
### **실제로 위의 예제와 같이 사용할 일은 극히 적다!** <br>
#### **주로 사용하게 되는 상황은...**

### **1. 디스플레이 크기가 5보다 큰 제품 선택**


```python
iphone_df[['디스플레이']] > 5 # 데이터프레임 # 이중 대괄호는 데이터프레임으로 출력(?)
```
...
```python
iphone_df.loc[iphone_df['디스플레이'] > 5] # df.loc 를 쓰고 한 버전(코드)
```
...
```python
iphone_df['디스플레이'] > 5 # 디스플레이 크기가 5보다 큰 제품 조회 - Series
```
...
```python
type(iphone_df['디스플레이'] > 5) # 위 결과값의 타입 확인
```
...
```python
iphone_df.loc[iphone_df['디스플레이'] > 5] # 디스플레이 크기가 5보다 큰 제품 인덱싱 - DataFrame
```
...
```python
type(iphone_df.loc[iphone_df['디스플레이'] > 5 ] ) # 위 결과값의 타임 확인
```
...
```python
# 본 코드 현재 없음. 공석 상태. # 각 제품의 Face ID 조회하는 코드
```


```python
iphone_df['Face ID'] == 'Yes' # Face ID 가 되는 제품 조회 - Series
```
...
```python
iphone_df.loc[iphone_df['Face ID'] == 'Yes'] # Face ID 가 되는 제품 인덱싱 - DataFrame
```
...
# **[20260421 Tue] - 오늘 수업 시작(2교시).**

### **4. 디스플레이가 5보다 크고, Face ID 가 되는 기종 검색(AND &)**


```python
# 본 코드 현재 없음. 공석 상태. # 디스플레이가 5보다 크고, Face ID 가 되는 제품 조회 - Series
```


```python
# 본 코드 현재 없음. 공석 상태. # 디스플레이가 5보다 크고, Face ID 가 되는 제품 조회 - DataFrame
```

### **4. 디스플레이가 5보다 크거나, Face ID 가 되는 기종 검색(OR |)**


```python
(iphone_df['디스플레이'] > 5) & (iphone_df['Face ID'] == 'Yes') # 디스플레이가 5보다 크거나, Face ID 가 되는 제품 조회 - Series
```
...
```python
iphone_df.loc[(iphone_df['디스플레이'] > 5) & (iphone_df['Face ID'] == 'Yes' )] # 디스플레이가 5보다 크거나, Face ID 가 되는 제품 조회 - DataFrame
```
...
**선생님께서 이 지금 하는것은 사실 새 파일에서(NEW ipynb) 시작하라 하였으므로 20260421tue_1.ipynb 를 만들어 거기서 다시 이것 해보자.**
